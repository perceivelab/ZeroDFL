...
class TrainDataManager:
    ...
    def __init__(
        ...
        elif dataname == 'skin_cancer':
            dataset = SkinCancer(cfg,available_classes,relabel=True)
...
class TestDataManager:
    ...
    def __init__(
        ...
        dataset_classnum = {'imagenet': 1000, 'caltech101':100, 'oxford_flowers': 102,'eurosat':10, 'oxford_pets':37, 'fgvc_aircraft': 100, 'skin_cancer': 9,
        ...
        for dataname in available_datasets:
            ...
            if dataname == 'caltech101':
                ...
                dataset = SkinCancer(cfg,available_classes)
...
class DatasetWrapper(TorchDataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(item.impath):
            try:
                img = Image.open(item.impath).convert("RGB")
            except Exception as e:
                raise RuntimeError(f"Failed to open image {item.impath}: {e}")
        else:
            raise FileNotFoundError(f"Image path does not exist: {item.impath}")
