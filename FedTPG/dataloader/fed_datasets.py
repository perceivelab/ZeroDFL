...
class SkinCancer(DatasetBase):
    ...
    def __init__(self, cfg, available_classes=None, relabel=True):
        self.data_name = "SkinCancer"
        root = os.path.abspath(os.path.expanduser(cfg.DATASET.ROOT))
        self.dataset_dir = os.path.join(root, self.dataset_dir)
        self.train_dir = os.path.join(self.dataset_dir, "Train")
        self.test_dir = os.path.join(self.dataset_dir, "Test")
        self.split_path = os.path.join(self.dataset_dir, "split_skin_cancer.json")
    ...
    def _create_split(self):
        """Create a train, val, test split and save it to a JSON file."""
        split_data = {"train": [], "val": [], "test": []}

        # Mappa delle classi a etichette univoche
        class_to_label = {class_name: idx for idx, class_name in enumerate(os.listdir(self.train_dir))}
        ...
        for class_name in os.listdir(self.train_dir):
            ...
            images = [os.path.join("Train", class_name, img) for img in os.listdir(class_path) if img.endswith(('.jpg', '.png', '.jpeg'))]
            train, val = train_test_split(images, test_size=0.2, random_state=42)
            split_data["train"].extend([[img, class_to_label[class_name], class_name] for img in train])
            split_data["val"].extend([[img, class_to_label[class_name], class_name] for img in val])
        ...
        for class_name in os.listdir(self.test_dir):
            ...
            images = [os.path.join("Test", class_name, img) for img in os.listdir(class_path) if img.endswith(('.jpg', '.png', '.jpeg'))]
            split_data["test"].extend([[img, class_to_label[class_name], class_name] for img in images])


        # Salva il JSON con le etichette corrette
