...
from federated.federation import Federated_Net
import wandb
...
def main(args):
    ...
    federated_net = Federated_Net(cfg, dataset=args.dataset, use_generator=args.use_generator)


    if args.eval_only:
        if args.model_name!='clip':
            federated_net.load_model(args.model_dir, epoch=args.load_epoch)
            
        # federated_net.local_test()
        if federated_net.cfg.TEST.SPLIT== 'base&new':
            federated_net.test("base")
            federated_net.test("new")
        else:
            federated_net.test(federated_net.cfg.TEST.SPLIT)
        return

    name_run = f"{args.dataset}_{cfg.MODEL.NAME}_Nc{cfg.TRAIN.NUM_CLASS_PER_CLIENT}_Seed{cfg.SEED}"
    
    wandb.init(project="federated-learning", name=name_run, 
               entity="amasano-universit-catania")

    

    if not args.no_train:
        if args.train_prompt:
            federated_net.train_prompt()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default=None, help="dataset to train and test with")
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_generator", action="store_true", help="generate prompts every 100 epochs"
    )
