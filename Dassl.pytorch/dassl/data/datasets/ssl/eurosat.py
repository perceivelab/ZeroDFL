...
class EuroSAT(DatasetBase):
    ...
    def __init__(self, cfg, nclients=3, iid=False, zero_shot=True, seed=43, relabel=True):
        ...
        """
        np.random.seed(seed)
        root = '/home/amasano/federated_coop/FedTPG/data'
        self.dataset_dir = os.path.join(root, self.dataset_dir)
        self.images_dir = os.path.join(self.dataset_dir, "2750")
        ...
        self.clients_data = self.split_data_among_clients(train, nclients, iid, seed=seed)
    ...
    def read_classnames(self):
        ...
        folders = sorted(f.name for f in os.scandir(self.images_dir) if f.is_dir())
    ...
    def load_data(self, classnames, seen_classes, novel_classes, train_relabeler, novel_relabeler):
        ...
        for folder in os.listdir(self.images_dir):
            class_path = os.path.join(self.images_dir, folder)
